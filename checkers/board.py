...
class Board:
    ...
    def get_valid_moves(self, piece):
        ...
        # Check if there is any opportunity to capture an opponent's checker
        for move, skipped in moves.items():
            if skipped:
                can_capture = True
                break

        # If there is an opportunity to capture an opponent's checker, only allow that move
        if can_capture:
            capture_moves = {}
            for move, skipped in moves.items():
                if skipped:
                    capture_moves[move] = skipped
